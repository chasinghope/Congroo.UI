// Modified by chasinghope (https://github.com/chasinghope/CurveLayoutGroup)
...
namespace Congroo.UI
{
    public class CurveLayoutGroup : LayoutGroup
    {
        ...
        [Range(2, 100)] public int NodeCount = 10;
        ...
        private void CalcAlongAxis(int axis)
        {
            ...
            {
                totalMin += rectChildren[0].sizeDelta[axis] * rectChildren[0].pivot[axis] *
                            rectChildren[0].localScale[axis];
                totalPreferred += rectChildren[0].sizeDelta[axis] * rectChildren[0].pivot[axis] *
                                  rectChildren[0].localScale[axis];

                totalMin += rectChildren[rectChildren.Count - 1].sizeDelta[axis] *
                            (1 - rectChildren[rectChildren.Count - 1].pivot[axis]) *
                            rectChildren[rectChildren.Count - 1].localScale[axis];
                totalPreferred += rectChildren[rectChildren.Count - 1].sizeDelta[axis] *
                                  (1 - rectChildren[rectChildren.Count - 1].pivot[axis]) *
                                  rectChildren[rectChildren.Count - 1].localScale[axis];
            }

            totalMin += (axis == 0 ? curveX_Factor * this.curveLength : curveY_Factor * this.curveHeight);
            totalPreferred += (axis == 0 ? curveX_Factor * this.curveLength : curveY_Factor * this.curveHeight) +
                              combinedPadding;
        ...
        private void SetChildrenAlongAxis_All(int axis)
        {
            ...
            for (int i = 0; i < rectChildren.Count; i++)
            {
                ...
                float param = axis == 0
                    ? this.CurveXAxis(this.MappingValue(i))
                    : -(this.curve.Evaluate(this.CurveXAxis(this.MappingValue(i))));
        ...
        private void SetChildrenAlongAxis_Type(int axis)
        {
            ...
            for (int i = 0; i < rectChildren.Count; i++)
            {
                ...
                float scaleFactor = child.localScale[axis];
                float param = axis == 0
                    ? this.CurveXAxis(i / (float)(NodeCount - 1))
                    : -(this.curve.Evaluate(this.CurveXAxis(i / (float)(NodeCount - 1))));
